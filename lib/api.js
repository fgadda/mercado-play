...
const BASE_URL = "https://play.mercadolibre.com.ar/api"

export const fetchFeedContent = async ({ offset = 0, filter = "" }) => {
  const limit = 24
  const url = `${BASE_URL}/feed?filter=${filter}&limit=${limit}&offset=${offset}`
  try {
...
export const fetchMovie = async ({ contentId }) => {
  const url = `${BASE_URL}/vcp/${contentId}?origin_content_id=${contentId}`
  try {
    const response = await fetch(url)
...
export const fetchContentDetail = async ({ contentId }) => {
  const url = `${BASE_URL}/content-detail?contentId=${contentId}`
  try {
    const response = await fetch(url)
    const { components } = await response.json()
    const {
